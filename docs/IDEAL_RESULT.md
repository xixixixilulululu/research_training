...
**现状：⚠️ 部分达到，试过三种改法，其中一种（扩大 val 比例）有效。**

| | run10（val 20%，F1 选） | run12（val 20%，F1 选，batch=32） | run18（val 20%，F-beta(2) 选） | run20（val 30%，F-beta(2) 选） |
|---|---|---|---|---|
| accuracy 均值 | 0.9168 | 0.9091 | 0.9109 | 0.9038 |
| accuracy 标准差 | ±0.0217 | ±0.0255 | ±0.0381 | **±0.0181** |

- **run12**：只把 `BATCH_SIZE` 从 16 改成 32，其他不变。结果标准差没降反升（±0.0255），均值也降了，
  说明"batch size 越大越稳"这个假设不成立。
- **run18**：把 checkpoint 选择从普通 F1 换成 `fbeta_score(beta=2, pos_label=malignant)`（跟 run14
  一致）后重新跑 5 折，标准差反而变大到 ±0.0381——追着"别漏诊恶性"这个更窄、更容易受单张图片影响
  的目标选 checkpoint，折间波动本身就更大。
- **run20**：在 run18 同样的 F-beta(2) 配置基础上，把 5 折诊断的 val 比例从 20% 扩大到 30%
  （`StratifiedShuffleSplit(test_size=0.3)`，5 次划分允许有重叠，不强求像 `StratifiedKFold` 那样
  严格不重叠）。**标准差从 ±0.0381 降到 ±0.0181，反而比 run10 用普通 F1 选 checkpoint 时的 ±0.0217
  还要小**——说明"扩大 val 比例"这个方向，至少在跟 run18 同口径对比时，确实有效果，原来假设的
  "已经是数据量天花板"不成立，值得保留这个方向继续用（比如正式训练时也考虑用更大的 val 比例）。

  **有一个没有完全控制住的变量要说清楚**：run20 同时换了两件事——val 比例（20%→30%）**和**切分方式
  （`StratifiedKFold` 严格不重叠 → `StratifiedShuffleSplit` 允许重叠）。理论上不重叠切分对"折间差异
  有多大"这个问题的估计更严格；允许重叠的切分，几次划分之间共享的样本更多，估计出来的标准差本身就会
  偏小一些，不完全是"val 变大"单独的功劳。所以这个结论算"方向对、效果真实存在，但具体多少是 val
  比例的贡献、多少是切分方式重叠的贡献，还没有拆开验证"，不是一个可以百分百归因的干净实验。
  另外 run20 的 5 折权重做 ensemble 后（`outputs/run20_ensemble_results.json`），recall 保持在
  0.8974（跟单模型一致，不像 run18 的 ensemble 那样把 recall 拉低到 0.79），还多救回了一个原本漏诊的
  案例（malignant (28).png），这也支持"更大 val 比例"这个方向是有实际收益的。
