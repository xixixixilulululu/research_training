...
| 理解训练相关概念：forward/backward propagation、epoch、loss function、optimizer、learning rate、batch size、overfitting/underfitting | ⛔ **未完成** | 同上；不过 Job 44 的实际结果里已经出现了一个真实的 overfitting 例子（见 README「跑过的记录」），可以拿它做例子去理解 |
...
| 画每个 epoch 的 train/test loss 曲线 | ✅ | `outputs/job49_loss_curve.png`（Job 44/49 基线，train vs test）；从 Job 50 开始逐 epoch 画的是 train vs **val**（`outputs/job{N}_loss_curve.png`），test 只在训练全部结束后单独评估一次（`outputs/job{N}_test_metrics.png`），避免逐 epoch 曲线里混进 test 数据 |
...
- **代码 / 环境 / 训练全部跑通**：数据、notebook、Slurm 提交、正式训练（Job 44）都已完成，有实际结果。
